class Gws::Gkan::Attendance::HolidayWorkFile
  include SS::Document
  ...
  field :expense, type: String, default: "compensate"
  field :comp_date, type: DateTime
  ...
  permit_params :name, :body, :expense, :comp_date,
  ...
  def expense_options
    I18n.t("gws/gkan.options.expense").map { |k, v| [v, k] }
  end
  ...
  def private_show_path
    ...
    url_helper.gws_gkan_attendance_holiday_work_file_path(site: site, id: id)
  end

  def workflow_wizard_path
    url_helper = Rails.application.routes.url_helpers
    url_helper.gws_gkan_attendance_holiday_work_wizard_path(site: site.id, id: id)
  end

  def workflow_pages_path
    url_helper = Rails.application.routes.url_helpers
    url_helper.gws_gkan_attendance_holiday_work_file_path(site: site.id, id: id)
  end
  ...
  def validate_comp_date
    if settlement?
      self.comp_date = nil
      return
    end
    self.errors.add :comp_date, :blank if comp_date.nil?
  end
