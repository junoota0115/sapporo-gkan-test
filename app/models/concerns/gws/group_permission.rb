module Gws::GroupPermission
  ...
  included do
    class_variable_set(:@@_permission_include_sub_groups, nil)
  ...
  def owned?(user)
    user = user.gws_user
    user_group_ids = self.class.permission_included_sub_groups? ? user.sub_group_ids : user.group_ids

    return true if (self.group_ids & user_group_ids).present?
  ...
  module ClassMethods
    ...
    def allow_condition(action, user, opts = {})
      user = user.gws_user
      user_group_ids = permission_included_sub_groups? ? user.sub_group_ids : user.group_ids
    ...
    def permission_included_sub_groups?
      class_variable_get(:@@_permission_include_sub_groups)
    end
    ...
    def permission_include_sub_groups
      class_variable_set(:@@_permission_include_sub_groups, true)
    end
